...
{ // whether two strings are equal case-sensitive
    if (strcmp(s1, s2) == 0)
        return true;
    return false;
}

bool image(const char *extension)
{ // whether the request is for some image file
...
{ // whether needle exists in haystack
...
{ // sets the folder from where the file is to be served
...
{                                   // sets the type of content to be returned according to request received
    ...
    lower(ext);
    if (!set_folder_name(ext, folder_name))
        return false;
    if (!image(ext))
...
bool getpath(const char *buffer_original, char *folder_name, char *file_name, char *content_type)
{
    ...
    if (equal(req_type, "GET") || equal(req_type, "OPTIONS") || equal(req_type, "HEAD"))
    {
        ...
        if (!set_content_type(file_name, folder_name, content_type))
            return false;
    }
    else
    {
...
void *handle_client(void *arg)
{
    ...
    while (1)
    {
        ...
        if (cnt == 0 || !buffer)
        { // nothing was received in allocated time
            close(newfd);
            continue;
        }
        ...
        if (strstr(file_name, "health_check"))
        { // this is health-check path, also used keep the service awake
